...
def load_data_evaluate(dir_path, dataset_type, n_sub, mode_evaluate="LOSO"):
    ...
    '''
    # Convert n_sub to string if it's not already
    n_sub = str(n_sub) if not isinstance(n_sub, str) else n_sub
...
def load_data_LOSO(dir_path, dataset_type, subject): 
    ...
    # Convert subject to int for comparison
    subject = int(subject) if isinstance(subject, str) else subject
...
def load_data(dir_path, dataset_type, n_sub, mode='train'):
    ...
    n_sub : str or int
        The number of subject, the scope range from 1 to 9.
    mode : str, optional
        The mode of data. The default is 'train'.
    ...
    # Convert n_sub to int for formatting
    n_sub_int = int(n_sub) if isinstance(n_sub, str) else n_sub
    
    file_path = os.path.join(dir_path, '{}{:02d}{}.mat'.format(dataset_type, n_sub_int, mode_s))
...
def numberClassChannel(database_type):
    """Return the number of classes and channels for specified EEG dataset type.
    
    This utility function provides quick access to dataset-specific configuration
    parameters, returning the appropriate number of classes and EEG channels
    based on the selected dataset type.
    
    Args:
        database_type: String indicating dataset type ('A' for BCI IV2a, 'B' for BCI IV2b)
    
    Returns:
        Tuple[int, int]: Number of classes and number of EEG channels
    
    Raises:
        ValueError: If an unsupported database_type is provided
    """
    if database_type=='A':
        number_class = 4
        number_channel = 22
