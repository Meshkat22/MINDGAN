...
Output folder: MINDGAN_exp_[timestamp]/
  01_sanity/
  02_ablation/  (6 sub-folders, one per config)
  03_full_model/
  figures/      (PNG + PDF versions of every figure)
  tables/
  MINDGAN_exp_summary.txt
"""

# ================================================================
# SECTION 0 — USER CONFIG  
# ================================================================
DATA_DIR      = r'./Preprocessed_BCI_IV_2A_dataset/'
DATASET_TYPE  = 'A'        # 'A' = 4-class BCI-IV 2A
...
ABLATION_EPOCHS= 1200       # per ablation config  (reduce to 200 if short on time)
...
class ExP:
    ...
    def load_data(self):
        ...
        print(f"    Sub {self.nsub}: train {tr_data.shape}  test {te_data.shape}")
    ...
    # ------------------------------------------------------------------
    def train(self):
        tr_data, tr_label, te_data, te_label = self.load_data()
        ...
        for epoch in range(self.n_epochs):
            ...
            for batch_eeg, batch_lbl in loader:
                n     = batch_eeg.size(0)
                n_val = max(1, int(VALIDATE_RATIO * n))
                tr_eeg = batch_eeg[:n-n_val].to(DEVICE).float()
                tr_lbl = batch_lbl[:n-n_val].to(DEVICE).long()
                val_eeg_l.append(batch_eeg[n-n_val:])
                val_lbl_l.append(batch_lbl[n-n_val:])
            ...
            # Validation
            self.clf.eval()
            val_eeg = torch.cat(val_eeg_l).to(DEVICE).float()
            val_lbl = torch.cat(val_lbl_l).to(DEVICE).long()
            with torch.no_grad():
                _, vl = self.clf(val_eeg)
            v_loss = self.criterion(vl, val_lbl).item()
            v_acc  = (vl.argmax(1) == val_lbl).float().mean().item()
...
plt.rcParams.update(MINDGAN_exp_STYLE)
...
def run_all():
    ...
    output_dir = f'MINDGAN_exp_{timestamp}'
...
FILES
  ...
  03_full_model/figures/         — all main figures
"""

    with open(os.path.join(output_dir, 'MINDGAN_exp_summary.txt'), 'w', encoding='utf-8') as f:
